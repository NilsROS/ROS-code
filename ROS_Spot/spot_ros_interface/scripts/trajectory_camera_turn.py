...
def heat_callback(data):
	...
	faces = face_cascade.detectMultiScale(gray, 1.2, 7) # image, scale factor: model with fixed size during training - so this size is detected in images- but by resizing the input image a large face can become a smaller one making it detectable to the algorithm , minimum number of neighbours.) ##outputRejectLevels = true??
...
def calculate_angle(x,y):
	angle = ((x-(hResolution/2))/(hResolution/2))* (hFieldOfView/2)
...
while not rospy.is_shutdown():
    ...
    print('{}\rKey pressed: {}\r'.format(' '*int(columns), numberOfFaces), end="")
